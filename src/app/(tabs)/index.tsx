            ...
            <View className='shadow bg-white flex-1 rounded-xl p-4 gap-3'>
              <View>
                <Text className='text-lg font-medium p-2'>
                  {titulo.filter((item: FaturaProps) => item.statusid == 1 && new Date(item.vencimento) < new Date()).length <= 0 ? 'Próxima Fatura' : 'Você possui ' + titulo.filter((item: FaturaProps) => item.statusid == 1 && new Date(item.vencimento) < new Date()).length + ' fatura(s) em aberto'}</Text>
