...
import { FaturaProps, ContratoProps } from '@/src/types/SgpTypes';
  ...
  return (
    <SafeAreaView className='bg-black'>
      <View className='bg-gray-100 h-full'>
        <View className='bg-slate-900 p-4 justify-center items-center relative'>
          ...
          <View className='bg-white m-4 py-6 absolute bottom-[-72px] left-0 right-0 shadow-md  rounded-xl p-8 flex-row justify-between'>
            <View className='flex-row items-center gap-2'>
              ...
              <Text className='text-slate-500 text-xl font-bold'>{data.status}</Text>
            </View>
            <View className='items-center'>
              <Text className='text-slate-900 text-xl font-medium'>CONTRATO</Text>
              <View className='flex-row items-center gap-2'>
                <MaterialCommunityIcons name='file-outline' color={'green'} size={32} />
        ...
        <View className='p-4 gap-2 mt-14'>
          <WifiStatus />
            ...
            <View className='shadow bg-white flex-1 rounded-xl p-4 gap-3'>
              <View>
                <Text className='text-lg font-medium p-2'>
                  {titulo.filter((item: FaturaProps) => item.statusid == 1 && new Date(item.vencimento) < new Date()).length <= 0 ? 'Nenhuma fatura em aberto' : 'Você possui ' + titulo.filter((item: FaturaProps) => item.statusid == 1 && new Date(item.vencimento) <= new Date()).length + ' fatura(s) em aberto'}</Text>

                <FlatList
                  horizontal
                  data={titulo.filter((item: FaturaProps) => item.statusid == 1 && new Date(item.vencimento).getMonth() <= new Date().getMonth() + 1)}
                  renderItem={({ item }) => (
                    <CardFaturaAbertas onPress={() => openBottomSheet(item)} item={item} />
                  )}
        ...
        >
          <View className='p-8'>
            <BottomSheetTextInput multiline={true} numberOfLines={3} className='bg-slate-100 rounded-2xl p-3 text-center border border-gray-200' value={list.codigopix ? list.codigopix : 'Essa fatura ainda não foi registrada, por favor entrar em contato com a central para solicitar o registro da fatura'} />
